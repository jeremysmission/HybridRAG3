...
8. [PM Cost Dashboard](#8-pm-cost-dashboard)
9. [Menu Bar](#9-menu-bar)
10. [Dark Mode and Light Mode](#10-dark-mode-and-light-mode)
11. [How to Run Your First Query](#11-how-to-run-your-first-query)
12. [How to Index a New Folder](#12-how-to-index-a-new-folder)
13. [Online vs Offline Mode](#13-online-vs-offline-mode)
14. [Keyboard Shortcuts](#14-keyboard-shortcuts)
15. [Common Problems and What They Mean](#15-common-problems-and-what-they-mean)
...
See [Section 10: Dark Mode and Light Mode](#10-dark-mode-and-light-mode) for
...
Open it from the menu bar: **Admin > PM Cost Dashboard...**

This opens a dedicated window that tracks API spending, token usage, and
data volume in real time. The dashboard is designed for program managers
and team leads who need visibility into how much each query costs and
whether the team is staying within budget. It auto-updates live after
each query -- no manual refresh needed.

### Big Numbers Row

The top row of the dashboard displays three large summary figures:

| Metric          | Description                                                  |
|-----------------|--------------------------------------------------------------|
| Session Spend   | Total dollar cost of all queries in the current GUI session. |
| Queries         | Number of queries submitted in the current session.          |
| Avg Cost/Query  | Session Spend divided by Queries.                            |

These values reset to zero each time you close and reopen the GUI. For
historical totals, see the Cumulative Team Stats section below.

### Budget Gauge

Below the big numbers is a horizontal gauge bar that shows how much of
your configured budget has been consumed. The gauge is color-coded:

| Fill percentage | Color  | Meaning                                  |
|-----------------|--------|------------------------------------------|
| Below 60%       | Green  | Spending is on track.                    |
| 60% to 85%      | Yellow | Approaching the budget limit.            |
| Above 85%       | Red    | Near or over budget -- review spending.  |

The budget target is set in `config/default_config.yaml` under the
`cost.budget` key. If no budget is configured, the gauge displays a
neutral gray bar with the label "No budget set".

### Token Breakdown Table

A table that breaks down costs by token direction:

| Column          | Description                                              |
|-----------------|----------------------------------------------------------|
| Direction       | **Input** (tokens sent to the LLM) or **Output** (tokens generated by the LLM). |
| Tokens          | Total token count for that direction in this session.    |
| Rate per 1M     | The cost rate per one million tokens (editable in the Rate Editor below). |
| Cost            | Tokens multiplied by the rate, displayed in dollars.     |

The bottom row shows the combined total across both directions.

### Data Volume Tracker

Shows the amount of raw data transferred during the session:

| Metric    | Description                                         |
|-----------|-----------------------------------------------------|
| KB Sent   | Kilobytes of prompt data sent to the LLM.           |
| KB Received | Kilobytes of response data received from the LLM. |
| KB Total  | Sum of sent and received.                           |

This is useful for estimating bandwidth consumption, especially on
metered connections or when working through a VPN.

### Cumulative Team Stats

While the big numbers and token table reset each session, the Cumulative
Team Stats section pulls historical totals from the local SQLite
database. It shows:
...
These values persist across GUI restarts because they are read from the
database, not from in-memory counters. The database file is stored
locally and is never sent over the network.

### Rate Editor

At the bottom of the dashboard, two editable fields let you adjust the
token pricing used for cost calculations:

- **Input rate per 1M tokens** -- the dollar cost per one million input
  tokens. Default matches your configured API provider pricing.
- **Output rate per 1M tokens** -- the dollar cost per one million output
  tokens.

After changing either rate, click the **Apply** button to recalculate
all session costs using the new rates. The Token Breakdown table,
Session Spend, and Avg Cost/Query update immediately.

**When to change rates:** Update these if you switch API providers, move
to a different model tier, or negotiate custom pricing. The rates do not
persist to disk -- they reset to the configured defaults on next launch.

### Export CSV Button

Click **Export CSV** to save the current session data to a CSV file. A
standard file-save dialog appears. The exported file includes:

- Timestamp of each query.
- Input tokens, output tokens, and cost per query.
- Session totals and cumulative stats.

This is useful for expense reporting, project cost tracking, and
post-session analysis.

### ROI Calculator

Below the cumulative stats, the dashboard includes an ROI Calculator section
with three live-updating big numbers: **TIME SAVED**, **VALUE SAVED**, and
**NET ROI**. Editable parameters (Hourly Rate defaulting to $48.44 per BLS
median PM wage, Team Size, and Minutes Saved Per Query) feed a team monthly
projection line showing estimated monthly savings vs API cost with ROI%.
Footnote citations at the bottom reference McKinsey, BLS OEWS, and HBR
research on AI-driven search time recovery.

### Savings Callout for Offline Queries

When queries are run in offline mode (using Ollama on localhost), the
dashboard displays a callout box that reads:

```
[OK] This query ran offline -- no API cost incurred.
```

Offline queries show $0.00 in the cost column. The dashboard tracks
how many queries ran offline versus online, so you can see how much
money the team saved by using local inference. The cumulative savings
figure appears at the bottom of the Cumulative Team Stats section.
...
- **Admin Settings...** -- Opens the engineering menu described in
  section 7.
- **PM Cost Dashboard...** -- Opens the cost tracking dashboard described
  in section 8.
...
## 15. Common Problems and What They Mean
