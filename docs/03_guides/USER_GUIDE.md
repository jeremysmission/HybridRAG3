...
9. [Cost Tracking (PM Cost Dashboard)](#9-cost-tracking-pm-cost-dashboard)
10. [REST API](#10-rest-api)
11. [Supported File Formats](#11-supported-file-formats)
12. [Tips for Better Answers](#12-tips-for-better-answers)
...
The Admin menu includes a test section at the bottom. Type a
question, click **Run Test**, and see the answer immediately. Useful
for testing the effect of slider changes.

---

## 9. Cost Tracking (PM Cost Dashboard)

The PM Cost Dashboard gives program managers and team leads visibility
into token usage and estimated costs across online API queries. Open it
from the GUI: **Admin** > **PM Cost Dashboard**.

The dashboard is read-only and does not affect query behavior. It only
tracks costs for online mode queries -- offline (Ollama/vLLM) queries
are free and do not appear.

### Session Tracking vs Cumulative Team Tracking

The dashboard has two tabs:

| Tab | What It Shows |
|-----|--------------|
| **Session** | Token usage and cost for the current GUI session only. Resets when you close and reopen the GUI. |
| **Cumulative** | All-time token usage across every session since the cost database was created. Persists across restarts. |

Use **Session** to monitor a single work session or spot a runaway
query. Use **Cumulative** to report total spend to management or track
team-wide usage over days or weeks.

### Reading the Token Breakdown

The center of the dashboard shows a token breakdown table:

| Column | Meaning |
|--------|---------|
| **Prompt tokens** | Tokens sent to the API (your question + retrieved document chunks). This is the largest contributor to cost. |
| **Completion tokens** | Tokens generated by the API (the answer text). Usually much smaller than prompt tokens. |
| **Total tokens** | Sum of prompt + completion. |
| **Estimated cost** | Dollar amount calculated from total tokens and the configured rate. |
| **Query count** | Number of queries executed in this tracking period. |

[WARN] Estimated cost is approximate. Actual billing depends on your
API provider's pricing, which may differ from the configured rate.

### Changing Token Rates

Default token rates match common API pricing, but you can adjust them
to match your provider:

1. In the PM Cost Dashboard, click **Edit Rates**
2. Enter the per-1K-token rate for prompt tokens
3. Enter the per-1K-token rate for completion tokens
4. Click **Apply**

Rates are saved to your local configuration and persist across
sessions. Changing rates recalculates all displayed cost estimates
immediately.

### Exporting Cost Data to CSV

To export cost data for reports or spreadsheets:

1. Open the PM Cost Dashboard (**Admin** > **PM Cost Dashboard**)
2. Select the **Session** or **Cumulative** tab depending on what you
   want to export
3. Click **Export CSV**
4. Choose a save location and filename
5. The exported file includes: date, query count, prompt tokens,
   completion tokens, total tokens, and estimated cost

The CSV file opens directly in Excel or any spreadsheet application.
Use it for monthly cost reports, budget reconciliation, or charge-back
tracking.

### Budget Gauge

The bottom of the dashboard shows a budget gauge -- a horizontal bar
that fills from left to right as cumulative cost approaches your
configured budget limit.

| Gauge Color | Meaning |
|-------------|---------|
| **Green** | Under 60% of budget -- normal usage |
| **Yellow** | 60-85% of budget -- monitor closely |
| **Red** | Over 85% of budget -- near or at limit |

To set or change the budget limit, click **Set Budget** in the
dashboard and enter the dollar amount. The gauge updates immediately.

### ROI Calculator

The dashboard includes an ROI Calculator that displays three live metrics:
TIME SAVED, VALUE SAVED, and NET ROI. Editable parameters -- Hourly Rate
(default $48.44, BLS median PM wage), Team Size, and Minutes Saved Per
Query -- feed a team monthly projection showing estimated savings vs API
cost with ROI%. Footnote citations reference McKinsey, BLS, and HBR
research on AI-driven search time recovery.

[WARN] The budget gauge is informational only. It does not block
queries when the budget is exceeded. If you need hard cost limits,
configure them at your API provider.
