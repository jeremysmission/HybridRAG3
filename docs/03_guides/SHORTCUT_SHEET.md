...
### Auto-Start (Opens PowerShell and You Are Ready to Go)

You can make HybridRAG load automatically every time you open PowerShell.
This means you skip the manual startup steps above -- just open a
terminal and all `rag-` commands work immediately.

**Step 1: You do NOT need to change any machine-wide settings.**

Corporate machines block scripts by default and group policy can override
your changes. The profile below uses `-Scope Process Bypass` which
temporarily allows scripts for that one PowerShell window only. It resets
when you close the window, so IT cannot flag it as a permanent policy
change.

**Step 2: Find out if you already have a PowerShell profile.**
...
If it says `True`, you already have one. If `False`, create it:
...
**Step 4: Paste these lines into Notepad, then save and close.**
...
| Line | What it does |
|------|-------------|
| `Set-ExecutionPolicy -Scope Process Bypass -Force` | Temporarily allows scripts to run for THIS PowerShell window only. Does not change the machine policy. Resets when you close the window. Corporate group policy cannot block this because Process scope overrides it. |
| `cd "D:\HybridRAG3"` | Goes to the project folder |
| `.\.venv\Scripts\Activate.ps1` | Activates the Python virtual environment (so `python` and `pip` use the right versions) |
| `. .\start_hybridrag.ps1` | Loads security lockdown, paths, and all `rag-` commands into the session |
| `Write-Host ...` | Prints a green confirmation message so you know it worked |

**Step 5: Close and reopen PowerShell.**

You should see all the `[OK]` messages from start_hybridrag.ps1 and then
"HybridRAG environment ready." in green. Type `rag-status` to verify.

**Work laptop note:** Change `D:\HybridRAG3` to wherever you extracted
the project on that machine. The paths inside `start_hybridrag.ps1`
also need updating (see Part 4 of INSTALL_AND_SETUP.md).

**To undo:** Open the profile again (`notepad $PROFILE`) and delete
the lines you added. Save and close.
