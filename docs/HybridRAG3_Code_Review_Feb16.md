...
## Baseline: GitHub repo `jeremysmission/HybridRAG3` (cloned fresh)
...
- Built-in knowledge base for ~30 model families (works behind proxy/air-gap)
...
- Air-gap deployment: copy `.model_cache/` folder to target machine
...
**Current state on Jeremy's machine (with backup config.py + backup YAML):**
...
2. `boot_hybridrag records warnings for missing Ollama` -- Ollama is running on Jeremy's machine, test expects it to be absent
...
1. **Restore hallucination guard config.py** -- Jeremy restored backup during debugging, needs to re-apply the delivery config.py
...
- The hallucination guard package was developed by a parallel Claude Opus session with a stale codebase snapshot. The corrected delivery was rebuilt by diffing against Jeremy's actual machine state.
