...
### Step 2: Verify (run from D:\HybridRAG3)
...
1. Copy `tools\rag-features.ps1` to `D:\HybridRAG3\tools\`
2. Copy `src\core\feature_registry.py` to `D:\HybridRAG3\src\core\`
...
|--------|-------------|-------------|
| `block` | Replaces answer with "I cannot verify this" | Production (defense) |
...
Copy these into D:\KnowledgeBase\LimitlessApp\:
...
|------|------------|-------|
| D:\HybridRAG3 (private) | YES | Full implementation |
| D:\HybridRAG3_Educational | YES | sync_to_educational.py handles sanitization |
| D:\KnowledgeBase\LimitlessApp | YES (regex only) | fact_extractor + claim_verifier |

Banned words removed: NGC, Northrop, Grumman, classified, ITAR,
CUI, clearance, Claude, Anthropic. Verified by SIM-17 sanitization tests.
