...
| Home only | mistral-small3.1:24b | 15 GB | 8-15 t/s | Excellent | 8-18 sec |

Tradeoffs:
- Bigger model = better answers = slower generation
- Each model size roughly halves the tokens/second
- On work workstation (12 GB VRAM): max is phi4:14b (9.1 GB)
- On home PC (48 GB VRAM, dual 3090): can run 24B models, or run 2 models in parallel
...
| Dual GPU (2 instances) | Home PC (dual 3090) | 2x throughput, user #8 waits 8-40 sec |
...
```
                    LAPTOP          WORK WORKSTATION  HOME PC
                    (8GB/512MB)     (64GB/12GB GPU)   (128GB/48GB GPU)
                    -----------     ----------------  ----------------
Embedder:
...
LLM:
  ...
  phi4:14b          No              Yes (max at 12GB) Yes
  mistral-nemo:12b  No              Yes               Yes
  mistral-small:24b No              No (>12GB)        Yes (one 3090)
  Two models at     No              No (single GPU)   Yes (two 3090s)
  once
...
| 5 | Dual GPU serving (home only) | Stage 5 | 2x concurrent throughput |
...
| RAM | ~2 GB | ~2 GB | No (work: 64 GB, home: 128 GB) |

The only real conflict is Ollama queuing. Both instances share one
Ollama server, so LLM requests are serialized. On the home PC with dual
3090s, you could run two Ollama instances on different GPUs to avoid this.
Work workstations (single GPU) cannot run parallel LLM instances.
