...
#   troubleshooting steps that Jeremy can follow.
# ============================================================================
...
class GoldenProbes:
    ...
    def run_all(self) -> List[ProbeResult]:
        ...
        results.append(self.probe_config_valid())

        # Group 2: Resources
        results.append(self.probe_disk_space())
        results.append(self.probe_memory_usage())
        ...
        if self.llm_router and hasattr(self.llm_router, 'api') and self.llm_router.api:
            results.append(self.probe_api_connectivity())

        # Group 4: Index integrity
        if self.vector_store:
            results.append(self.probe_index_readable())
            results.append(self.probe_index_not_empty())

        # Group 5: Embedding
        if self.embedder:
            results.append(self.probe_embedder_loaded())
            results.append(self.probe_embedding_dimensions())

        # Group 6: Retrieval (only if all prerequisites are available)
        if self.vector_store and self.embedder:
            results.append(self.probe_golden_query())

        return results

    # -- Configuration Probes -----------------------------------------------

    def probe_config_valid(self) -> ProbeResult:
        """
        ...
        try:
            ...
            elif self.config.mode not in ("offline", "online"):
                errors.append(f"config.mode='{self.config.mode}' is invalid")

            if not hasattr(self.config, "ollama"):
                errors.append("config.ollama section is missing")

            if not hasattr(self.config, "api"):
                errors.append("config.api section is missing")

            if hasattr(self.config, "chunking"):
                if self.config.chunking.chunk_size < 100:
                    errors.append(
                        f"chunk_size={self.config.chunking.chunk_size} is too small (<100)"
                    )
                if self.config.chunking.overlap >= self.config.chunking.chunk_size:
                    errors.append(
                        "overlap >= chunk_size (chunks would have no unique content)"
                    )

            if hasattr(self.config, "retrieval"):
                if self.config.retrieval.top_k < 1:
                    errors.append(f"top_k={self.config.retrieval.top_k} must be >= 1")
                if not (0.0 <= self.config.retrieval.min_score <= 1.0):
                    errors.append(
                        f"min_score={self.config.retrieval.min_score} must be 0.0-1.0"
                    )
    ...
    def probe_disk_space(self) -> ProbeResult:
        """
        ...
        start = time.time()
        try:
            # Check the drive where index lives (default: current directory)
            index_dir = getattr(self.config, "paths", None)
            check_path = "."
    ...
    def probe_memory_usage(self) -> ProbeResult:
        """
        PROBE: Is the Python process using too much RAM?

        THRESHOLD: Warn at 80% of available system RAM.

        WHY: The embedding model (~100MB) + index data + query processing
        can accumulate. If we hit swap space, everything slows to a crawl.
        """
        start = time.time()
        try:
            import resource
            # Get the peak RSS (Resident Set Size) in KB
            usage_kb = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
            usage_mb = usage_kb / 1024  # Convert to MB

            latency = (time.time() - start) * 1000

            # On Windows, ru_maxrss might not be available
            # (resource module is Unix-only), so this probe
            # gracefully degrades on Windows
            return ProbeResult(
            ...
            # Fall back to psutil if available, otherwise skip
            try:
                import psutil
    ...
    def probe_api_connectivity(self) -> ProbeResult:
        """
        PROBE: Can we reach the cloud API endpoint?

        NOTE: This does NOT send a real query (that would cost money).
        It only checks if the endpoint is reachable via a lightweight
        request.
        ...
        try:
            ...
            # Most API endpoints return 401 or 405 for unauthenticated GET
            # requests -- that's fine, it means the server is reachable
    ...
    def probe_index_readable(self) -> ProbeResult:
        """
        ...
        try:
            ...
            # Try to get chunk count -- this exercises the DB connection
            count = 0
    ...
    def probe_index_not_empty(self) -> ProbeResult:
        """
    ...
    def probe_embedder_loaded(self) -> ProbeResult:
        """
        ...
        try:
            ...
            result = self.embedder.embed_batch(test_text)
    ...
    def probe_embedding_dimensions(self) -> ProbeResult:
        """
        ...
        try:
            ...
            results = self.embedder.embed_batch(test_texts)
    ...
    def probe_golden_query(self) -> ProbeResult:
        """
        ...
        start = time.time()
        try:
            # Use a very generic query that should match something
            golden_query = "system architecture design"

            if hasattr(self.vector_store, "search"):
                from .retriever import Retriever
                retriever = Retriever(self.vector_store, self.embedder, self.config)
                results = retriever.search(golden_query)
            else:
                # Fallback: just try embedding the query
                embedding = self.embedder.embed_batch([golden_query])
                results = []  # Can't search without proper vector store interface
...
class FaultAnalysisEngine:
    ...
    def report_fault(
        ...
        Args:
            ...
            severity_override: Override the auto-classified severity
