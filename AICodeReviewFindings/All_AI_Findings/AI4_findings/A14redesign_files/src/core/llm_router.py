...
class APIRouter:
    ...
    def __init__(self, config: Config, api_key: str, endpoint: str = "",
        ...
        # If the app has not run the boot pipeline yet, the NetworkGate
        # singleton will still be in OFFLINE mode (fail-closed default).
        # For unit tests and for programmatic usage, we auto-configure
        # the gate when the config explicitly says we're in ONLINE mode.
        try:
            gate = get_gate()
            if getattr(config, 'mode', '').lower() == 'online' and gate.mode_name == 'offline':
                from .network_gate import configure_gate
                configure_gate('online', api_endpoint=self.base_endpoint)
        except Exception:
            # Never let gate initialization break API router creation; the per-call
            # checks still enforce policy.
            pass
