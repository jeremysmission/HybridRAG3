...
class HttpResponse:
    ...
    def json(self) -> dict:
        ...
        Contract (by design):
            - Return a dict for valid JSON objects.
            - Return {} for empty body or invalid JSON.

        Diagnostics:
            If parsing fails or content-type is suspicious, details are stored
            in `self.json_diagnostic` so callers can still do forensics without
            breaking the common "dict-or-empty" expectation used throughout the codebase.
        """
        diag = self.json_diagnostic
        return diag.get("_parsed", {})
    ...
    def json_diagnostic(self) -> dict:
        ...
        Fields:
            _ok: bool
            _parsed: dict (only when _ok True)
            _reason: str
            _content_type: str
            _status_code: int
            _body_preview: str
        ...
        # Empty body: common for HEAD/204/etc
        if not body.strip():
            return {
                "_ok": False,
                "_reason": "empty_body",
        ...
        # Non-JSON content-type: captive portals / proxies / WAF HTML
        ...
        try:
            ...
            # Normalize: only dicts count as "json()" results; other JSON types go into diagnostic
...
def create_http_client(config_dict=None) -> HttpClient:
    ...
    # Environment kill switch (defense in depth):
    # NetworkGate is the primary source of truth, but we also honor the env var here
    # so standalone tools/tests behave predictably.
    env_offline = os.environ.get("HYBRIDRAG_OFFLINE", "").strip().lower()
    if env_offline in {"1", "true", "yes", "on"}:
