...
def normalize_config(config: dict) -> dict:
    """Normalize older config shapes into the current schema.

    This repo has had multiple config iterations. To keep the program portable
    and to avoid breaking older config files, we treat the YAML as an *input*
    and then normalize to a stable in-memory schema.

    Current in-memory schema (minimum):
        config['api']['provider']      in {'azure','openai'}
        config['api']['auth_scheme']   in {'api-key','bearer'}
        config['api']['key']           placeholder string (never an actual secret)
        config['http']                 dict with timeout/max_retries/retry_delay/etc
    """
    cfg = dict(config or {})
    ...
    # Provider detection (back-compat): if provider missing, infer from endpoint.
    provider = (api.get('provider') or '').strip().lower()
    if not provider:
        # crude but effective patterns (kept identical to ApiClientFactory heuristics)
        lower = endpoint.lower()
        if 'azure' in lower or 'aoai' in lower or 'cognitiveservices' in lower:
            provider = 'azure'
        else:
            provider = 'openai'
    ...
    auth_scheme = (api.get('auth_scheme') or '').strip().lower()
    if not auth_scheme:
        auth_scheme = 'api-key' if provider == 'azure' else 'bearer'
    api['auth_scheme'] = auth_scheme

    # Never store real keys in YAML. Keep a placeholder so schema validation passes.
    key_val = api.get('key')
    if not key_val:
        api['key'] = '${ENV:HYBRIDRAG_API_KEY}'

    cfg['api'] = api

    # HTTP section (older configs put timeouts under api.*).
    http_cfg = dict(cfg.get('http') or {})
    if 'timeout' not in http_cfg and 'timeout_seconds' in api:
        http_cfg['timeout'] = int(api.get('timeout_seconds') or 30)
    http_cfg.setdefault('max_retries', 2)
    http_cfg.setdefault('retry_delay', 1.0)
    http_cfg.setdefault('verify_ssl', True)
    cfg['http'] = http_cfg
